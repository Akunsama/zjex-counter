export default {
  PHONE: /^1(3|4|5|7|8)\d{9}$/,
  //IDCARD: /^\d{6}((?:19|20)((?:\d{2}(?:0[13578]|1[02])(?:0[1-9]|[12]\d|3[01]))|(?:\d{2}(?:0[13456789]|1[012])(?:0[1-9]|[12]\d|30))|(?:\d{2}02(?:0[1-9]|1\d|2[0-8]))|(?:(?:0[48]|[2468][048]|[13579][26])0229)))\d{2}(\d)[xX\d]$/,
	CHANANAME: /^[\u4e00-\u9fa5]{2,4}$/,
	EMAIL: /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/,
	CARLICENSE : /^[\u4e00-\u9fa5]{1}[A-Z]{1}[A-Z_0-9]{5}$/,
	FRAMENO : /^[A-Z]{1}[A-Z0-9]{15}[0-9]{1}$/,
	ENGINENO : /^[a-zA-Z0-9]{6,30}$/,
	SEATS : /^[1-9]$/,
	EXHAUSTSCALE : /^[+]{0,1}(\d+)$|^[+]{0,1}(\d+\.\d+)$/,
	NOTEMPTY : /\S+/,
  PASSWORD: /^[a-zA-Z]\w{7,19}$/,
  USERNAME: /^[\da-zA-Z\u4E00-\u9FA5]{1,10}$/,
  //NAME: /^[\da-zA-Z\u4E00-\u9FA5]{1,10}$/,
  COMPANY: /^[\da-zA-Z\u4E00-\u9FA5]{1,25}$/,
  SHORTHAND: /^[\da-zA-Z\u4E00-\u9FA5]{1,10}$/,
  NODENAME: /^[\da-zA-Z\u4E00-\u9FA5]{0,50}$/,
  NODEDISC: /^(.|\n){0,150}$/,
  FAX:/^(\d{3,4}-)?\d{7,8}$/,
  TEL:/^0\d{2,3}-?\d{7,8}$/,
  TWODECIMALPLACES:/^(\d+(\.\d{2}))?$/,
  NUMBER:/^[0-9]*$/,
  SECNO:/^\d{10}$/,
  //经办人姓名 10个字符
  NAME:/^[\da-zA-Z\u4E00-\u9FA5]{1,60}$/,
  //质权人姓名 60个字符
  PLEDGEENAME:/^[\da-zA-Z\u4E00-\u9FA5]{1,60}$/,
  //身份证件号码 字母或数字均可，40个字符
  IDCARD:/^[\da-zA-Z]{1,40}/,
  //预警线、平仓线 整数，1-999
  LINE:/^\+?[1-9][0-9]{0,2}$/,
  //融资利率 0~999.99 保留两位小数
  FINANCINGRATE:/^(?=0|.*[1-9])\d{0,3}(?:\.\d{0,2})?$/,
  //融资金额 0~9999999999999.99  保留两位小数
  FINANCINGAMOUNT:/^(?=0|.*[1-9])\d{0,13}(?:\.\d{0,2})?$/,
  //质押登记编号 100个字符
  PLEDGEREGISTRATION:/^[\da-zA-Z]{1,100}/,
}
