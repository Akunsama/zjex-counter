...
class formService {
  constructor() {
    ...
    this.idcard = function (val) {
      return this.check(null,inputInfo.IDCARD,val);
    }
    this.fax = function (val) {
      return this.check(reg.FAX, inputInfo.FAX, val);
    }
    this.tel = function (val) {
      return this.check(reg.TEL, inputInfo.TEL, val);
    }
    this.notEmpty = function(val){
      return this.check(reg.NOTEMPTY, inputInfo.NOTEMPTY, val)
    };
    this.TwoDecimalPlaces = function(val){
      return this.check(reg.TWODECIMALPLACES, inputInfo.TWODECIMALPLACES, val)
    };
    this.number = function(val){
      return this.check(reg.NUMBER, inputInfo.NUMBER, val)
    };
    this.secNo = function (val) {
      return this.check(reg.SECNO, inputInfo.SECNO, val)
    }
  }
