...
class Key {
	constructor() {
		this.path = 'C:\\Users\\guozhikun\\keystores'; //钱包默认保存的路径
		this.keyObject = {}; //钱包对象
		this.keyList = []; //钱包对象列表
...
//					return item.account;					
	 ...
	 * (*)address 传入的地址，用于区别钱包文件
	 */
	login(username, password,address, type, successCB, errorCB) {
		this._updateKeyList();
		if(username && password) {
			if(type == 1) {
				let keyObject = null;
				for(let i = 0; i < this.keyList.length; i++) {
					if(this.keyList[i].address == address) {
